...
def predict():
    ...
    t1.delete("1.0", END)
    t1.insert(END, Name.get() + ' ' + gender.get() + ' ' + str(num1.get())
              + ' ' + str(num2.get() + num3.get() + num4.get() + num5.get() + num6.get() + num7.get()))
...
S1 = Entry(root, textvariable=num1)
S1.grid(row=6, column=1)

S2 = Entry(root, textvariable=num2)
S2.grid(row=8, column=1)

S3 = Entry(root, textvariable=num3)
S3.grid(row=9, column=1)

S4 = Entry(root, textvariable=num4)
S4.grid(row=10, column=1)

S5 = Entry(root, textvariable=num5)
S5.grid(row=11, column=1)

S6 = Entry(root, textvariable=num6)
S6.grid(row=12, column=1)

S7 = Entry(root, textvariable=num7)
S7.grid(row=13, column=1)

dst = Button(root, text="Predict",
             command=predict, bg="teal", fg="white")
dst.config(font=("Helvetica", 15, "bold"))
dst.grid(row=8, column=3, padx=10)
dst.grid(row=9, column=3, padx=10)

t1 = Text(root, height=1, width=40, bg="white", fg="Black")
t1.config(font=("Ariel", 16, "bold"))
t1.config(state='disable')
